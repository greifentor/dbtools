...
public class LiquibaseFileModelReader implements ModelReader {
	 ...
	 * @param rootFile
	 *            The root file of the Liquibase model.
	...
	private DatabaseChangeLog getDatabaseChangeLog() throws Exception {
		...
		return ChangeLogParserFactory.getInstance()
				...
				.parse(this.rootFile.getName(), changeLogParameters,
						resourceAccessor);
	}
		...
		for (ChangeSet changeSet : changeLog.getChangeSets()) {
			for (Change change : changeSet.getChanges()) {
				if (change instanceof CreateTableChange) {
					...
					tables.add(this.factory.createTable(ctc.getTableName(),
							columns, new ArrayList<>()));
				} else {
					System.out.println(
							"ignored: " + change.getClass().getSimpleName());
				}
			}
		}
	...
	private TypeInfo getDataType(ColumnConfig cc) {
		LiquibaseDataType type = DataTypeFactory.getInstance()
				.fromDescription(cc.getType(), null);
		TypeInfo ti = new TypeInfo().setName(type.getName().toUpperCase());
		if ("int".equalsIgnoreCase(type.getName())
				|| "integer".equalsIgnoreCase(type.getName())) {
			ti.setName("INTEGER");
			ti.setDataType(Types.INTEGER);
		} else if ("number".equalsIgnoreCase(type.getName())
				|| "numeric".equalsIgnoreCase(type.getName())) {
			ti.setName("NUMERIC");
			ti.setDataType(Types.NUMERIC);
			ti.setColumnSize(
					Integer.valueOf(type.getParameters()[0].toString()));
			ti.setDecimalDigits(
					Integer.valueOf(type.getParameters()[1].toString()));
